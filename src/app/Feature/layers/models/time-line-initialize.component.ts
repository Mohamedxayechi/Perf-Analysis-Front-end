// timeline-initializer.component.ts
...
@Component({
  selector: 'app-timeline-initializer',
  imports:[],
  standalone: true,
  template: ``,
  styles: ``,
})
export class TimelineInitializerComponent implements OnInit {
  private readonly TIMELINE: Timeline = this.createTimeline();
  ...
  ngOnInit(): void {
    this.emitTimelineInitialize();
  }

  private emitTimelineInitialize(): void {
    const engine = Engine.getInstance();
    if (engine.isInitialized) {
      // console.log(
      //   `[${new Date().toISOString()}] TimelineInitializer: Emitting timeline.initialize with ${
      //     this.TIMELINE.layers.length
      //   } layers`
      // );
      engine.emit({
        type: 'timeLine.initialize',
      ...
      // console.log(
      //   `[${new Date().toISOString()}] TimelineInitializer: Engine not initialized, retrying in 100ms`
      // );
      setTimeout(() => this.emitTimelineInitialize(), 100);
    }
  }
}