...
export class Display implements OnDestroy {
  ...
  private totalTime = 0;
  private cursorX = 0;
  private distancePerTime = 50; // Pixels per second, matches ResizableDirective's timePerWidth
  ...
  private minScale: number = 0.1; // Matches ZoomComponent template's minScale
  private maxScale: number = 2;
  private stepScale: number = 0.1;
  ...
  private emitEvent(event: EventPayload): void {
    console.log(
      `[${new Date().toISOString()}] Display: Emitting event: ${event.type}, origin: ${
        event.origin
      }, data:`,
      event.data
    );
  ...
  handleEvent(event: EventPayload): void {
    console.log(
      `[${new Date().toISOString()}] Display: Received event: ${event.type}, origin: ${
        event.origin
      }, data:`,
      event.data
    );
    if (this.eventProcessing || event.processed) {
      console.warn(
        `[${new Date().toISOString()}] Display: Skipped event: ${event.type}, eventProcessing: ${
    ...
    try {
      const handlers: { [key: string]: (data: any) => void } = {
        ...
        'media.resized': (data) => this.handleResize(data?.index, data?.time, data?.width),
        ...
        'parameters.distancePerTimeUpdated': (data) =>
          this.handleDistancePerTimeUpdate(data?.distancePerTime || this.distancePerTime),
        // Added zoom event handlers
        'zoom.in': (data) => this.handleZoomIn(data),
        'zoom.out': (data) => this.handleZoomOut(data),
        'zoom.change': (data) => this.handleZoomChange(data),
        'zoom.get': () => this.handleZoomGet(),
      };
  ...
  private handleResize(index: number | undefined, time: number | undefined, width: number | undefined): void {
    ...
    this.emitEvent({
      ...
      data: { index, time, width, updatedMedias: result.updatedMedias }, // Include width
      origin: 'domain',
    });
  }
  ...
  // Added zoom event handlers
  private handleZoomIn(data: any): void {
    const stepScale = data?.stepScale ?? this.stepScale;
    if (this.zoom < this.maxScale) {
      this.zoom = parseFloat((this.zoom + stepScale).toFixed(1));
      console.log(
        `[${new Date().toISOString()}] Display: Zoom increased to ${this.zoom}`
      );
      this.emitZoomChanged();
    }
  }

  private handleZoomOut(data: any): void {
    const stepScale = data?.stepScale ?? this.stepScale;
    if (this.zoom > this.minScale) {
      this.zoom = parseFloat((this.zoom - stepScale).toFixed(1));
      console.log(
        `[${new Date().toISOString()}] Display: Zoom decreased to ${this.zoom}`
      );
      this.emitZoomChanged();
    }
  }
  ...
  private handleZoomChange(data: any): void {
    const zoom = data?.zoom ?? this.zoom;
    const minScale = data?.minScale ?? this.minScale;
    const maxScale = data?.maxScale ?? this.maxScale;
    this.zoom = Math.max(minScale, Math.min(maxScale, parseFloat(zoom.toFixed(1))));
    this.minScale = minScale;
    this.maxScale = maxScale;
    console.log(
      `[${new Date().toISOString()}] Display: Zoom changed to ${this.zoom}, minScale: ${this.minScale}, maxScale: ${this.maxScale}`
    );
    this.emitZoomChanged();
  }

  private handleZoomGet(): void {
    console.log(
      `[${new Date().toISOString()}] Display: Emitted current zoom ${this.zoom}`
    );
    this.emitZoomChanged();
  }
  ...
  private handleImportMedia(file?: File): void {
    ...
    if (file.type.startsWith('video')) {
      this.getVideoThumbnail(file).then(({ thumbnail, duration }) => {
        ...
        console.error(
          `[${new Date().toISOString()}] Display: Failed to import video: ${file.name}`,
          err
        )
      );
      ...
      console.error(
        `[${new Date().toISOString()}] Display: Unsupported file type: ${file.type}`
      );
    }
  }
  ...
  private handleMediaReordered(medias: Media[] | undefined): void {
    ...
    console.log(
      `[${new Date().toISOString()}] Display: Media list reordered`,
      { count: medias.length, medias: medias.map(this.summarizeMedia) }
    );
    this.emitEvent({
      type: 'media.imported',
  ...
  private handleCursorChange(cursorX: number): void {
    ...
    const globalSecond = cursorX / (this.distancePerTime * this.zoom); // Adjusted for zoom
    this.state.currentTime = globalSecond;
    console.log(
      `[${new Date().toISOString()}] Display: Cursor changed to ${cursorX}, globalSecond: ${globalSecond}, zoom: ${this.zoom}`
    );
    this.emitEvent({
      type: 'cursor.updated',
      data: {
  ...
  private togglePlayPause(currentSecond?: number): void {
    console.log(
      `[${new Date().toISOString()}] Display: togglePlayPause`,
      {
        currentSecond,
  ...
  private pausePlayback(): void {
    console.log(
      `[${new Date().toISOString()}] Display: pausePlayback`,
      { isPlaying: this.state.isPlaying }
    );
    if (this.updateTimer) {
    ...
    if (this.video && this.currentMediaIndex >= 0) {
      ...
      this.lastPausedTime =
        this.video.currentTime + MediaModel.calculateAccumulatedTime(this.currentMediaIndex);
      ...
      console.log(
        `[${new Date().toISOString()}] Display: Paused image at globalSecond: ${this.lastPausedTime}`
      );
      mediaElement = this.currentImage;
      width = this.currentImage.width;
      height = this.currentImage.height;
      currentTime =
        this.state.currentTime - MediaModel.calculateAccumulatedTime(this.currentMediaIndex);
    }
    ...
    this.emitEvent({
      type: 'render.frame',
      data: { mediaElement, width, height, currentTime, zoom: this.zoom }, // Include zoom
      origin: 'domain',
      processed: false,
    });
  }
  ...
  private playFromSecond(globalSecond: number): void {
    console.log(
      `[${new Date().toISOString()}] Display: playFromSecond`,
      { globalSecond, medias: this.medias.map(this.summarizeMedia) }
    );
    ...
    console.log(
      `[${new Date().toISOString()}] Display: Playing media at index ${result.index}, localSecond: ${result.localSecond}`
    );
  ...
  private stopPlayback(): void {
    console.log(
      `[${new Date().toISOString()}] Display: stopPlayback`,
      { isPlaying: this.state.isPlaying }
    );
    if (this.updateTimer) {
      clearTimeout(this.updateTimer);
    ...
    this.emitEvent({
      type: 'render.frame',
      data: { mediaElement: null, zoom: this.zoom }, // Include zoom
      origin: 'domain',
      processed: false,
    });
  }
  ...
  private renderVideo(
    ...
    if (!media.video?.match(/^(blob:|\/assets\/videos\/)/)) {
      console.error(
        `[${new Date().toISOString()}] Display: Invalid video URL for ${media.label}, url: ${
          media.video
        }`
      );
    ...
    const handleMetadata = () => {
      console.log(
        `[${new Date().toISOString()}] Display: Video metadata loaded: ${media.label}`,
        {
      ...
      if (!isFinite(seekTime) || seekTime < 0) {
        seekTime = 0;
        console.warn(
          `[${new Date().toISOString()}] Display: Clamped seekTime to 0 for ${media.label}`
        );
      } else if (seekTime > actualEndTime) {
        seekTime = actualEndTime;
        console.warn(
          `[${new Date().toISOString()}] Display: Clamped seekTime from ${seekTime} to ${actualEndTime} for ${media.label}`
        );
      }

      this.video!.currentTime = seekTime;
      this.state.currentTime = timing.accumulatedBefore + seekTime;
      this.cursorX = this.state.currentTime * this.distancePerTime * this.zoom; // Adjusted for zoom
      console.log(
      ...
      this.video!.play()
        ...
        .catch((err) => {
          console.error(
            `[${new Date().toISOString()}] Display: Video play failed for ${media.label}, src: ${
              media.video
            }`,
            err.message
          );
    ...
    this.video.addEventListener('error', (e) => {
      console.error(
        `[${new Date().toISOString()}] Display: Video error for ${media.label}, src: ${media.video}`,
        this.video?.error,
        e
      );
      this.tryNextMedia(index + 1, timing.accumulatedBefore + timing.duration);
    ...
    this.video.addEventListener('ended', () => {
      console.log(
        `[${new Date().toISOString()}] Display: Video ended event for ${media.label}`,
  ...
  private startVideoLoops(
    ...
    const renderFrame = () => {
      if (
        ...
        console.log(
          `[${new Date().toISOString()}] Display: Stopping render loop for ${media.label}`,
          {
            isPlaying: this.state.isPlaying,
      ...
      this.emitEvent({
        ...
        data: {
          mediaElement: this.video,
          width: this.video.videoWidth * this.zoom, // Scale dimensions
          height: this.video.videoHeight * this.zoom,
          currentTime: this.video.currentTime,
          zoom: this.zoom,
        },
        origin: 'domain',
    ...
    const updateCursor = () => {
      if (!this.state.isPlaying || !this.video || this.video.paused) {
        console.log(
          `[${new Date().toISOString()}] Display: Stopping cursor update for ${media.label}`,
          {
            isPlaying: this.state.isPlaying,
            paused: this.video?.paused,
            ended: this.video?.ended,
      ...
      this.cursorX = currentGlobalSecond * this.distancePerTime * this.zoom; // Adjusted for zoom

      console.log(
        `[${new Date().toISOString()}] Display: Updating cursor for ${media.label}`,
        {
          currentTime: this.video.currentTime,
  ...
  private renderImage(
    ...
    console.log(
      `[${new Date().toISOString()}] Display: Setting up image for ${media.label}`,
      { src: media.image }
    );
    if (media.isThumbnailOnly) {
      console.warn(
        `[${new Date().toISOString()}] Display: Skipping image rendering for ${media.label} as it is marked thumbnail-only`,
        { src: media.image }
      );
      this.tryNextMedia(index + 1, timing.accumulatedBefore + timing.duration);
      return;
    ...
    image.onload = () => {
      ...
      const updateImageTimer = () => {
        ...
        this.emitEvent({
          ...
          data: {
            mediaElement: image,
            width: image.width * this.zoom, // Scale dimensions
        ...
        if (currentLocalSecond >= timing.duration) {
          console.log(
            `[${new Date().toISOString()}] Display: Image ended: ${media.label}`,
            { nextIndex: index + 1 }
          );
          this.tryNextMedia(index + 1, timing.accumulatedBefore + timing.duration);
        } else {
  ...
  private tryNextMedia(nextIndex: number, globalSecond?: number): void {
    console.log(
      `[${new Date().toISOString()}] Display: Trying next media`,
      { nextIndex, globalSecond }
    );
